...
const ChatByIdPage = () => {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      const unreadMessages = messages.filter((message) => !message.leido)
      markAsRead(unreadMessages.map((message) => message.id))
    }
  }, [messages, markAsRead])
  ...
  return (
    <div className="w-full min-h-screen flex flex-col bg-gray-50">
      {/* Header */}
      <div className="flex items-center justify-between p-4 bg-gray-main border-b border-gray-200 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-1">
            <span className="text-lg font-semibold">
              {messages[0]?.remitente.nombre} {messages[0]?.remitente.apellido}
            </span>
      ...
      <div className="bg-gray-main border-t border-gray-200 p-4">
        <form onSubmit={handleSend} className="flex items-center gap-3">
          <div className="flex-1 relative">
            ...
            >
              <Icon icon="material-symbols:send-rounded" className="text-sm" />
            </Button>
          </div>
        </form>
      </div>
